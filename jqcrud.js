  ...
  $.ajax({
    ...
    success: function (response) {
      ...
      for (var i = 0; i < response.length; i++) {
        var rec = response[i];
        recipes.append(
          `<div class="recipe" data-id="${rec.id}"><h3>Name: ${rec.name}</h3><p>Email: ${rec.email}</p></p><p><button class="btn btn-danger" >DELETE</button><button class="btn btn-warning">EDIT</button></p></div>`
        );
