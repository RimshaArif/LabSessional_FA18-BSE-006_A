  ...
  $.ajax({
    ...
    success: function (response) {
      ...
      for (var i = 0; i < response.length; i++) {
        var rec = response[i];
        recipes.append(
          `<div class="recipe" data-id="${rec.id}"><h3>${rec.name}</h3><p><button class=" btn-sm float-right">Delete</button><button class=" btn-sm float-right">Edit</button> ${rec.email}</p></div>`
        );
